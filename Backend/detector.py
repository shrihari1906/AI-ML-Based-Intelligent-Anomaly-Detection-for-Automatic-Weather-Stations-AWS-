"""
detector.py — ML-based anomaly detection for AWS sensor readings.

Uses a trained IsolationForest model (anomaly_model.pkl) combined with
domain-specific physical checks for comprehensive anomaly detection.

Function signature:
    detect_anomaly(readings: list[dict]) -> dict

Input:  list of recent sensor readings (dicts), most recent last.
        Each dict has: temperature, humidity, pressure, wind_speed

Output: {"is_anomaly": bool, "score": float, "reason": str}
"""
...
if not os.path.exists(MODEL_FILE):
    raise FileNotFoundError(
        f"Trained model '{MODEL_FILE}' not found. Please run train_model.py first."
    )
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in BASE_FEATURES:
        X[f"{col}_rolling_std_5"] = (
            df[col].rolling(window=5, min_periods=1).std().fillna(0.0)
        )
        X[f"{col}_diff_from_prev"] = df[col].diff().fillna(0.0)
        X[f"{col}_rolling_mean_5"] = (
            df[col].rolling(window=5, min_periods=1).mean().bfill().fillna(0.0)
        )
...
def detect_anomaly(readings: list) -> dict:
    """
    ...
    Parameters:
        readings (list[dict]): A chronological list of recent sensor readings
                               (e.g. 5–10 readings), where the last item is
                               the current reading to evaluate. Each dict has
                               keys: temperature, humidity, pressure, wind_speed
    ...
    temp_rolling_std = float(
        latest_feature_vector.get(
            "temperature_rolling_std_5", pd.Series([1.0])
        ).iloc[0]
    )
    temp_diff = float(
        latest_feature_vector.get(
            "temperature_diff_from_prev", pd.Series([0.0])
        ).iloc[0]
    )
    hum_diff = float(
        latest_feature_vector.get(
            "humidity_diff_from_prev", pd.Series([0.0])
        ).iloc[0]
    )

    # --- Temporal pattern flags ---
    # Stuck sensor: constant value for >= 5 readings
    is_stuck = len(readings) >= 5 and (
        temp_rolling_std < 1e-4
        or all(r.get("temperature") == temp for r in readings[-5:])
    )
    is_dropout = humidity <= 0.0 or (hum_diff < -50.0 and humidity < 10.0)
    is_spike = (
        temp > 45.0
        or temp < -10.0
        or abs(temp_diff) > 25.0
        or wind_speed > 25.0
    )

    # Flag anomaly if IsolationForest detects outlier OR domain check triggers
    ...
    if is_stuck:
        reasons.append(
            f"stuck sensor (temperature constant at {temp} °C for >= 5 readings)"
        )
    if is_dropout:
...
if __name__ == "__main__":
    ...
    res = detect_anomaly(normal_window)
    print(f"\n[Test 1 - Normal] {normal_window[-1]}")
    print(f"Result: {res}")
    ...
    res = detect_anomaly(stuck_window)
    print(f"\n[Test 2 - Stuck Sensor] {stuck_window[-1]}")
    print(f"Result: {res}")
    ...
    res = detect_anomaly(spike_window)
    print(f"\n[Test 3 - Temp Spike] {spike_window[-1]}")
    print(f"Result: {res}")
    ...
    res = detect_anomaly(dropout_window)
    print(f"\n[Test 4 - Humidity Dropout] {dropout_window[-1]}")
    print(f"Result: {res}")
